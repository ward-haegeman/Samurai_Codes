...
#include "flux_6eqs_int_energy.hpp"
...
void Relaxation<dim>::init_variables() {
  ...
  samurai::for_each_cell(mesh,
                         [&](const auto& cell)
                         {
                           ...
                           conserved_variables[cell][ALPHA1_RHO1_E1_INDEX] = conserved_variables[cell][ALPHA1_RHO1_INDEX]*EOS_phase1.e_value(rho1[cell], p1[cell]);;

                           conserved_variables[cell][ALPHA2_RHO2_E2_INDEX] = conserved_variables[cell][ALPHA2_RHO2_INDEX]*EOS_phase2.e_value(rho2[cell], p2[cell]);;
  ...
  samurai::make_bc<samurai::Dirichlet<1>>(conserved_variables, alpha1L, alpha1L*rho1L, alpha2L*rho2L, (alpha1L*rho1L + alpha2L*rho2L)*velL,
                                                                        alpha1L*rho1L*EOS_phase1.e_value(rho1L, p1L),
                                                                        alpha2L*rho2L*EOS_phase2.e_value(rho2L, p2L))->on(left);
  samurai::make_bc<samurai::Dirichlet<1>>(conserved_variables, alpha1R, alpha1R*rho1R, alpha2R*rho2R, (alpha1R*rho1R + alpha2R*rho2R)*velR,
                                                                        alpha1R*rho1R*EOS_phase1.e_value(rho1R, p1R),
                                                                        alpha2R*rho2R*EOS_phase2.e_value(rho2R, p2R))->on(right);
}
...
void Relaxation<dim>::update_pressure_before_relaxation() {
  samurai::for_each_cell(mesh,
                         [&](const auto& cell)
                         {
                           const auto e1 = conserved_variables[cell][ALPHA1_RHO1_E1_INDEX]/conserved_variables[cell][ALPHA1_RHO1_INDEX]; /*--- TODO: Add treatment for
                                                                                                                                               vanishing volume fraction ---*/

                           const auto e2 = conserved_variables[cell][ALPHA2_RHO2_E2_INDEX]/conserved_variables[cell][ALPHA2_RHO2_INDEX]; /*--- TODO: Add treatment for
                                                                                                                                               vanishing volume fraction ---*/
  ...
  samurai::for_each_cell(mesh,
                         [&](const auto& cell)
                         {
                           ...
                           if(start_energy_update_phase_1) {
                             // Update the internal energy of phase 1
                             const auto e1 = EOS_phase1.e_value(conserved_variables[cell][ALPHA1_RHO1_INDEX]/conserved_variables[cell][ALPHA1_INDEX], p_star);
                             /*--- TODO: Add treatment for vanishing volume fraction ---*/

                             conserved_variables[cell][ALPHA1_RHO1_E1_INDEX] = conserved_variables[cell][ALPHA1_RHO1_INDEX]*e1;

                             if(preserve_energy) {
                               // Update the internal energy of phase 2 imposing internal energy conservation during the relaxation
                               conserved_variables[cell][ALPHA2_RHO2_E2_INDEX] = rhoe_0 - conserved_variables[cell][ALPHA1_RHO1_E1_INDEX];
                             }
                             else {
                               // Update the internal energy of phase 2
                               const auto e2 = EOS_phase2.e_value(conserved_variables[cell][ALPHA2_RHO2_INDEX]/(1.0 - conserved_variables[cell][ALPHA1_INDEX]), p_star);
                               /*--- TODO: Add treatment for vanishing volume fraction ---*/
